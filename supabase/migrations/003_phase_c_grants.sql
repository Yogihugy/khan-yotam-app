-- Phase C — chat + POI grants; seed hardcoded Khan as read-only POI row (optional display source)

GRANT SELECT, INSERT, UPDATE ON public.messages TO authenticated;
GRANT SELECT ON public.poi TO authenticated;
GRANT SELECT ON public.duty_officer TO authenticated;

-- Ensure Khan Yotam appears in POI list (Phase C read-only display; admin can edit later)
